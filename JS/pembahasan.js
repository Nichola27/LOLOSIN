...
const SUBTES_NAMES = {
    ...
    "pbm": "TPS - Pemahaman Baca & Menulis",
...
function loadQuestion() {
    ...
    // TAMBAHKAN KODE INI: Atur tombol submit di top nav
    const topSubmitBtn = document.getElementById("submit-btn"); // Pastikan ID ini sama dengan tombol di top nav
    if (topSubmitBtn) {
        // Tampilkan hanya di soal terakhir (index terakhir)
        if (currentQuestion === soal.length - 1) {
            topSubmitBtn.style.display = "block";
        } else {
            topSubmitBtn.style.display = "none";
        }
    }
    
    // Update tombol submit di bottom navigation
